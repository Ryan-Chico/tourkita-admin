...
import { collection, doc, getDocs, setDoc, deleteDoc, serverTimestamp, onSnapshot } from 'firebase/firestore';
...
const UserManagement = () => {
        ...
        const unsubscribeUsers = onSnapshot(usersRef, (snapshot) => {
            const registeredUsers = snapshot.docs.map(doc => {
                ...
                return {
                    ...
                    registeredDate: data.createdAt?.toDate() || null, // Store as Date object
                };
            });
    ...
    const filteredUsers = useMemo(() => {
        return users.filter(user => {
            ...
            const userDate = user.registeredDate ? new Date(user.registeredDate) : null;
            if (!userDate) return false; // Filter out users without a valid date if filtering by date
            ...
            const matchesDateFrom = fromDate ? userDate >= fromDate : true;
            const matchesDateTo = toDate ? userDate <= toDate : true;

            return matchesSearch && matchesFilter && matchesDateFrom && matchesDateTo;
        }).sort((a, b) => (b.registeredDate || 0) - (a.registeredDate || 0));
    }, [users, search, viewFilter, dateFrom, dateTo]);


    const totalUsers = users.filter(u => u.status !== 'archived').length;
    const registeredCount = users.filter(u => u.status === 'registered').length;
    const guestCount = users.filter(u => u.status === 'guest').length;
    const archivedCount = users.filter(u => u.status === 'archived').length;
    const onlineCount = users.filter(u => u.status !== 'archived' && u.activeStatus).length;
    const offlineCount = users.filter(u => u.status !== 'archived' && !u.activeStatus).length;
                            ...
                            <tbody>
                                {loading ? (
                                    Array.from({ length: 10 }).map((_, i) => (
                                        <SkeletonTableRow key={i} columns={visibleColumnCount} />
                                    ))
                                ) : (
                                        ...
                                        filteredUsers.map((user) => (
                                            <tr key={user.id}>
                                                ...
                                                {columnVisibility.activeStatus && (
                                                    <td><span style={{ color: getStatusColor(user.activeStatus) }}>{user.activeStatus ? 'Online' : 'Offline'}</span></td>
                                                )}
                                                {columnVisibility.userType && <td>{user.status === 'registered' ? user.userType || 'N/A' : '—'}</td>}
                                                {columnVisibility.registeredDate && <td>{user.registeredDate ? user.registeredDate.toLocaleDateString() : 'N/A'}</td>}
                                                {columnVisibility.actions && (
                                                    <td>
                                                        {user.status === 'registered' && (<button className="archive-btn" onClick={() => handleWarnAndArchive(user.id)}>Archive</button>)}
                                                        {user.status === 'archived' && (<button className="archived-btn" disabled>Archived</button>)}
                                                    </td>
                                                )}
                                            </tr>
                                        ))
                                    ) : (
                                        <tr><td colSpan={visibleColumnCount} className="no-data">No users found.</td></tr>
                                    )
                                )}
                            </tbody>
                        </table>
