...
const ArManagement = () => {
    ...
    const handleDeleteClick = async (e, asset) => {
        ...
        setIsLoading(true);
        try {
            // STEP 1: Update the primary 'markers' document first.
            const markerToUpdate = markers.find(m => m.name === asset.id);
            if (markerToUpdate) {
                // The document ID in 'markers' can be a number, so convert it to a string.
            ...
            // STEP 2: Delete the Firestore documents from other AR collections.
            await deleteDoc(doc(db, "arTargets", asset.id));
            await deleteDoc(doc(db, "arMarkers", asset.id));

            // STEP 3: After all database operations succeed, clean up Storage.
            // This filter is the key fix for the "indexOf" error.
            ...
            alert(`An error occurred: ${error.message}. The data may be in an inconsistent state.`);
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="dashboard-main">
            ...
            <div className="dashboard-section">
                <div className="page-header">
                    ...
                    <p className="page-subtitle">Manage 3D models and content for locations in Obando.</p>
                </div>
                ...
                {isLoading ? <p>Loading assets...</p> : (
                    <div className="markers-list">
                        {arAssets.map((asset) => (
                            <div className="marker-card" key={asset.id} onClick={() => handlePreviewClick(asset)}>
                                ...
                                <div className="card-actions">
                                    <button onClick={(ev) => handleEditClick(ev, asset)} className="edit-btn">Edit</button>
                                    <button onClick={(ev) => handleDeleteClick(ev, asset)} className="delete-btn">Delete</button>
                                </div>
                            </div>
                        ))}
                    </div>
