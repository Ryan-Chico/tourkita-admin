...
const ARUploadModal = ({ markers, assetToEdit, onClose }) => {
    ...
    // ✅ CORRECTED AND SAFER SUBMIT LOGIC
    const handleSubmit = async (e) => {
        ...
        const oldFileUrls = {
            image: isEditMode && assetToEdit.imageUrl ? assetToEdit.imageUrl : null,
            model: isEditMode && assetToEdit.modelUrl ? assetToEdit.modelUrl : null,
            video: isEditMode && assetToEdit.videoUrl ? assetToEdit.videoUrl : null,
        };
        ...
        try {
            ...
            // STEP 3: After DB success, delete old files from Storage if they were replaced
            setStatusMessage("Cleaning up old files...");
            const cleanupPromises = [];
            if (newImageUrl && oldFileUrls.image) cleanupPromises.push(deleteObject(ref(storage, oldFileUrls.image)));
            if (newModelUrl && oldFileUrls.model) cleanupPromises.push(deleteObject(ref(storage, oldFileUrls.model)));
            if (newVideoUrl && oldFileUrls.video) cleanupPromises.push(deleteObject(ref(storage, oldFileUrls.video)));
