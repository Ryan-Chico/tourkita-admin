...
const ARUploadModal = ({ markers, assetToEdit, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // Step 1: Upload all new files to Storage
            setStatusMessage("Uploading new files...");
            const newImageUrl = await uploadToFirebaseStorage(newFiles.image, 'models/markers');
            const newModelUrl = await uploadToFirebaseStorage(newFiles.model, 'models/models');
            const newVideoUrl = await uploadToFirebaseStorage(newFiles.video, 'models/video');
            ...
            // Step 2: Update all relevant Firestore documents
            setStatusMessage("Updating database records...");
            const arTargetRef = doc(db, "arTargets", formData.location);
            const arMarkerRef = doc(db, "arMarkers", formData.location);
            const selectedMarker = markers.find(m => m.name === formData.location);

            await setDoc(arTargetRef, finalData, { merge: true });
            await setDoc(arMarkerRef, { name: formData.location, imageUrl: finalData.imageUrl }, { merge: true });
            ...
            // Step 3: After successful DB update, delete old files from Storage
            setStatusMessage("Cleaning up old files...");
            const cleanupPromises = [];
            if (newImageUrl && oldFileUrls.image) {
                cleanupPromises.push(deleteObject(ref(storage, oldFileUrls.image)));
            }
            if (newModelUrl && oldFileUrls.model) {
                cleanupPromises.push(deleteObject(ref(storage, oldFileUrls.model)));
            }
            if (newVideoUrl && oldFileUrls.video) {
                cleanupPromises.push(deleteObject(ref(storage, oldFileUrls.video)));
            }
