...
const ARUploadModal = ({ markers, arAssets, assetToEdit, onClose }) => {
    ...
    const availableMarkers = useMemo(() => {
        ...
        if (formData.category === 'Building') {
            ...
            return markers.filter(marker => !locationsWithBuildings.includes(marker.name));
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // --- LOGIC FOR BUILDINGS ---
            if (formData.category === 'Building') {
                // Use the location name as the unique ID for the building asset
                const targetRef = doc(db, "arTargets", formData.location);
                await setDoc(targetRef, finalData);

                // Add the modelUrl directly to the marker document
                await updateDoc(markerRef, {
                    arCameraSupported: true,
                    modelUrl: finalData.modelUrl
                });
            }
            // --- LOGIC FOR RELICS/ARTIFACTS ---
            else if (formData.category === 'Relics/Artifacts') {
                // Use a new unique ID for new relics, or the existing one if editing
                ...
                // Use dot notation to add/update the artifact in the 'artifacts' map
                await updateDoc(markerRef, {
                    arCameraSupported: true,
                    [`artifacts.${targetRef.id}`]: artifactPayload
                });
            }

            // Update the target image for the location (common for both categories)
            ...
            if (validFilesToClean.length > 0) {
                await Promise.all(validFilesToClean.map(url => deleteObject(ref(storage, url))));
            }
