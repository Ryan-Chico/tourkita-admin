...
const ARUploadModal = ({ markers, onClose }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // --- 1. Create or update the document in the 'arTargets' collection (This is unchanged) ---
            ...
            // --- 2. Update the document in the 'markers' collection ---
            const selectedMarker = markers.find(m => m.name === formData.location);
            if (selectedMarker) {
                const markerRef = doc(db, "markers", selectedMarker.id);

                // --- THIS IS THE ONLY CHANGE ---
                // Now updates two fields in the 'markers' document
                await updateDoc(markerRef, {
                    arCameraSupported: true,
                    modelUrl: modelUrl // Add the modelUrl here as well
                });
            }
