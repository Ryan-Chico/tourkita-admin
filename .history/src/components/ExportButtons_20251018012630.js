...
const ExportButtons = ({ users, columnVisibility }) => {
    ...
    const columnConfig = {
        ...
        registeredDate: {
            ...
            accessor: (user) => {
                ...
                // Check if the value is a true Date object
                if (date instanceof Date) {
                    return date.toLocaleDateString();
                }
                // If it's already a string or something else, just return it
                return date || 'N/A';
            }
        },
    };

    // 3. Get an array of keys for only the visible columns.
    ...
    const handleExportPDF = () => {
        ...
        // 4. Dynamically build the table headers for the PDF.
        const tableColumn = ["No", ...visibleColumnKeys.map(key => columnConfig[key].header)];

        // 5. Dynamically build the table rows based on visible columns.
    ...
    const handleExportExcel = () => {
        // 6. Dynamically create the data objects for Excel.
