...
const ARUploadModal = ({ markers, onClose }) => {
    ...
    const uploadToCloudinary = async (file, type) => {
        ...
        if (type === "image") {
            ...
            preset = "ar_raw";
            resourceType = "raw"; // for audio + 3D models
        }
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (error.response) {
                ...
                alert(`Upload failed: ${error.response.data.error?.message || "Check console for details."}`);
            } else if (error.request) {
                console.error("Upload failed with no response:", error.request);
                alert("Upload failed: No response from Cloudinary.");
            } else {
